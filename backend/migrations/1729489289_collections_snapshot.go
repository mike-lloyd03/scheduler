		...
		jsonData := `[
			{
				...
				"updated": "2024-10-21 03:29:58.437Z",
				"name": "users",
				"type": "auth",
				"system": false,
				"schema": [
				...
				"indexes": [],
				"listRule": "@request.auth.id != \"\" && ( id = @request.auth.id || @request.auth.orgs:each ?= orgs:each)",
				"viewRule": "@request.auth.id != \"\" && ( id = @request.auth.id || (@collection.permissions.user.id ?= @request.auth.id && (orgs:each ?= @collection.permissions.org || groups:each ?= @collection.permissions.group)))",
				"createRule": "@request.auth.id != \"\" && (@collection.permissions.user.id ?= @request.auth.id && orgs:each ?= @collection.permissions.org)",
				"updateRule": "@request.auth.id != \"\" && (id = @request.auth.id || @collection.permissions.user.id ?= @request.auth.id && orgs:each ?= @collection.permissions.org)",
				"deleteRule": "@request.auth.id != \"\" && (@collection.permissions.user.id ?= @request.auth.id && orgs:each ?= @collection.permissions.org)",
				"options": {
					"allowEmailAuth": true,
					"allowOAuth2Auth": false,
					"allowUsernameAuth": true,
					"exceptEmailDomains": null,
					"manageRule": "@request.auth.id != \"\" && (@collection.permissions.user.id ?= @request.auth.id && orgs:each ?= @collection.permissions.org)",
					"minPasswordLength": 8,
			...
			},
			{
				...
				"updated": "2024-10-21 03:29:58.435Z",
			...
			},
			{
				...
				"updated": "2024-10-21 03:29:58.436Z",
				"name": "orgs",
				"type": "base",
				"system": false,
				"schema": [
			...
			},
			{
				...
				"updated": "2024-10-21 05:31:42.981Z",
				"name": "permissions",
				"type": "base",
				"system": false,
				"schema": [
